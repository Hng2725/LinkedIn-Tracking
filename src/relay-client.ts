...
/** Obtain OAuth access token via client_credentials grant, with retries */
async function getAccessToken(privosUrl: string, clientId: string, clientSecret: string): Promise<string> {
	...
	for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
		try {
			...
			const delay = Math.min(5000 * attempt, 60000); // 5s, 10s, 15s... max 60s
			console.error(`[Relay] OAuth attempt ${attempt}/${MAX_RETRIES} failed: ${err.message}. Retrying in ${delay / 1000}s...`);
...
export async function connectRelay(opts: RelayClientOptions): Promise<WebSocket> {
	...
	ws.on('close', (code) => {
		...
		setTimeout(async () => {
			try {
				await connectRelay(opts);
			} catch (err: any) {
				console.error(`[Relay] Reconnect failed: ${err.message}. Will retry in 30s...`);
				setTimeout(() => connectRelay(opts).catch(() => {}), 30000);
			}
		}, 5000);
	});
